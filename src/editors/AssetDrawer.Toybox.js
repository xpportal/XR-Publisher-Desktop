...
function AssetDrawerToybox(editor) {
	...
	function updateAssetsGrid() {
		...
		assets.forEach(asset => {
			...
			item.dom.addEventListener('mouseenter', () => {
				item.dom.style.transform = 'translateY(-3px)';
				// glow pulse effect
				item.dom.style.boxShadow = '0 0 10px 2px #ffffff1a';
			});
			...
			header.dom.className = 'bg-[#222222] rounded-t-lg h-7 w-11/12 pb-1 pt-0 pl-2 pr-2 pt-2 pb-3 text-xs font-bold mb-0 mx-auto text-white text-left';
			header.dom.style.cssText = `
				max-width: 124px;
				`;
			...
			statsBar.dom.innerHTML = `
				<p class="text-xs text-white">${(asset.filesize / 1000000).toFixed(3)} MB</p>
				<p class="text-xs text-white">Poly: ${asset.polycount ? asset.polycount.toFixed(0) : '0'}</p>
			`;
    ...
    loadMoreButton.dom.className = 'button bg-violet-500 text-white px-4 py-2 rounded-full text-sm mt-0 mb-12 ml-6';
