...
function Loader(editor) {
    ...
    // Add new method to handle URLs
	this.loadFromUrl = async function(url, assetType) {
		try {
			...
			// If we know the asset type, handle it directly
			if (assetType) {
				// force .glb for now
				const extension = 'glb';

				const fakeFile = new File([blob], `temp.${extension}`, { type: blob.type });
				this.loadFile(fakeFile);
			} else {
