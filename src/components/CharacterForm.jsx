...
export const CharacterForm = ({ character, onChange, onSubmit }) => {
	...
	return (
		<div className="space-y-6">
			...
			<div className="space-y-4">
				<h3 className="text-white text-sm text-lg font-semibold">API Configuration</h3>

				<div>
					<label className="block text-white text-sm mb-2">OpenAI API Key</label>
					<input
						type="password"
						className="w-full bg-gray-800 text-white text-sm p-2 rounded"
						value={character.settings?.secrets?.openai || ''}
						onChange={(e) => onChange({
							...character,
							settings: {
								...character.settings,
				...
				<div>
					<label className="block text-white text-sm mb-2">Anthropic API Key</label>
					<input
						type="password"
						className="w-full bg-gray-800 text-white text-sm p-2 rounded"
						value={character.settings?.secrets?.anthropic || ''}
						onChange={(e) => onChange({
							...character,
							settings: {
								...character.settings,
